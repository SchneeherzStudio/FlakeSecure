/**
 * ============================================================================
 * FlakeSecure - Browser Extension Content Script (Chrome / Chromium)
 * ============================================================================
 * 
 ...
 * 
 * 6. SOCKET.IO RELAY COMMUNICATION:
 *    - connectSocket(sessionId, keyHex, onData): Connects to the relay server via Socket.IO, joins the session room, and receives encrypted payloads from the mobile app.
 * 
 ...
 *    - removeOverlay(cancelled): Dismisses the overlay with exit animation, disconnects socket, and resets state.
...
(function () {
  ...
  function fillRegistrationFields(fieldsData) {
    ...
    if (fieldsData.phone) {
      const phoneInput = document.querySelector('input[type="tel"], input[name*="phone" i], input[name*="telefon" i], input[autocomplete="tel"]');
  ...
  function connectSocket(sessionId, keyHex, onData) {
    ...
    socket.on('connect', () => {
      ...
      if (typeof chrome !== 'undefined' && chrome.storage) {
        chrome.storage.local.get(['authToken'], (res) => {
          socket.emit('join-session', { sid: sessionId, token: res.authToken, domain });
        });
      } else {
    ...
    socket.on('login-data', async (payload) => {
      console.log('[FlakeSecure] Received payload from app');
    ...
    socket.on('totp-data', async (payload) => {
      console.log('[FlakeSecure] Received TOTP data from app');
      const decrypted = await decryptData(payload, keyHex);
